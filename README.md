...
- [环境要求](#环境要求)
- [快速开始](#快速开始)
- [Agent 使用说明（最新链路）](#agent-使用说明最新链路)
- [视频生成模式](#视频生成模式)
- [关键接口速览](#关键接口速览)
- [接口调用示例](#接口调用示例)
- [运行测试](#运行测试)
...
**Q: 切换编辑脚本 / 编辑视频时为什么现在不跳顶部了？**  
A: Agent 三栏工作区已改为锚定当前视频卡附近，聊天列独立滚动，便于在同一条结果上下文中继续编辑。

**Q: 为什么我在对话区看不到视频预览，页面直接被压成三栏？**  
A: 当前默认是「先聊天预览、后点击进入编辑」：只有点击视频卡下方的「编辑视频 / 编辑脚本」按钮时才进入双栏/三栏。若仍异常，建议刷新页面并确认前端已更新到最新代码。

**Q: 首页为什么看起来和之前不一样？**  
A: 首页已升级为更偏工作台式的 Landing：大屏比例、Prompt 输入区、右侧视觉卡片和整体层次都做了统一优化，但主流程没有变化。
