...
| Agent | 商品洞察、提示词增强、多模型视频生成、并发任务跟踪、弹窗选图 | ✅ 可用 |
...
| `/api/veo/chain` | POST | 链式扩展生成（8/16/24s） |
...
**Q: 文字蒙版未生效？**  
A: 当前 ffmpeg 可能不含 `drawtext` 过滤器，系统自动降级但仍可导出其他编辑项。
...
- [ ] 任务中心完善（筛选、重试、失败诊断）
- [ ] 端到端 UI 自动化测试（Agent + Studio 关键链路）
- [ ] Docker/Compose 标准化本地与演示部署
- [ ] 多语言提示词优化（英/日/阿拉伯语市场）
- [ ] Veo 3.0 正式版 + Grok 原生 duration 参数支持（视模型 API 更新）
