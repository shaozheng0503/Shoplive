...
- **兼顾体验与稳定性**：支持流式输出、轮询退避、任务并发、超时续轮询和失败回退
...
| 模块 | 能力 | 当前状态 |
| --- | --- | --- |
| Landing | 快速输入诉求、跳转 Agent 工作流 | ✅ 可用 |
| Agent | 商品洞察、提示词增强、8s/16s 视频生成、并发任务跟踪 | ✅ 可用 |
| Studio | 时间线编辑、异步渲染、进度/取消、统计与优化建议 | ✅ 可用（MVP+） |
| Image Lab | 商品相关生图与管线衔接 | ✅ 可用 |
| Backend API | Veo / Shoplive / Agent / Media / Video Edit 全链路接口 | ✅ 可用 |
...
> 你可以把截图放到 `docs/images/` 后，按下面文件名直接展示在 README。

| 页面 | 建议截图文件 | 说明 |
| --- | --- | --- |
| Landing | `docs/images/landing.png` | 输入诉求、上传商品图的入口页 |
| Agent | `docs/images/agent.png` | 商品洞察、提示词增强、视频任务并发面板 |
| Studio | `docs/images/studio.png` | 时间线编辑、异步渲染、进度与取消 |
| Image Lab | `docs/images/image-lab.png` | 商品相关生图与管线入口 |
...
- `LITELLM_API_KEY` 已配置
- Google 凭据文件可访问（默认读取 `credentials/...json`，也可通过 `GOOGLE_APPLICATION_CREDENTIALS` 覆盖）
...
- [ ] 将 Studio 时间线从 MVP 升级为可插拔渲染后端（ffmpeg / cloud render）
- [ ] 增加更完整的任务中心（筛选、重试、失败诊断）
- [ ] 补充端到端 UI 自动化测试（Agent + Studio 关键链路）
- [ ] 引入 Docker/Compose 标准化本地与演示部署
- [ ] 发布正式 License 与贡献者指南模板

## 如何贡献

欢迎通过 Issue / PR 参与改进。

1. Fork 本仓库并创建功能分支：`feat/<your-topic>`
2. 提交前确保通过测试：`python3 -m pytest backend/tests/ -v`
3. PR 描述建议包含：背景、改动点、验证方式、风险与回滚方案
4. 若涉及接口变更，请同步更新 `README` 与 `api/openapi.json` 对应说明

推荐提交信息风格（与当前仓库保持一致）：

- `Improve ... with ...`
- `Enhance ... reliability and UX ...`
- `Update README ...`
